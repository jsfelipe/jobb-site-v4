...
import { submitTesteGratis } from '@/services/testeGratisApi';
import {
  INTERESSE_OPTIONS,
...
export default function TesteGratis() {
  ...
  // --- Etapa 2 / criação automática de domínio (desativado — reativar no futuro) ---
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.status && clienteId) {
        // Fluxo atual: só cadastra no JobbAdmin (WhatsApp segue no backend)
        navigate('/teste-gratis/sucesso', { replace: true });
        return;

        // --- Fluxo antigo: ir para etapa 2 (domínio/login/senha) ---
        ...
        <section className="w-full max-w-4xl">
          ...
          >
            <div className="flex flex-col justify-end bg-gradient-to-br from-[#171717] to-[#232323] p-8 text-right text-white md:w-2/5 md:rounded-l-2xl rounded-t-2xl md:rounded-tr-none">
              ...
              <p className="mt-2 text-lg text-white/80">Aproveite!</p>
              {/* {step === 2 && (
            ...
            <div className="bg-white p-6 md:w-3/5 md:rounded-r-2xl rounded-b-2xl md:rounded-bl-none">
              ...
              {/* --- Etapa 2: domínio / login / senha (desativado) ---
              {step === 2 && (
